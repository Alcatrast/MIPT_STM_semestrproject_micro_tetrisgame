...
/* USER CODE BEGIN PV */
uint16_t adc_vals[2] = {0};  // [0]=X (PA0), [1]=Y (PA1)
int joystick_mode = 1;       // 1 = джойстик активен, 0 = клавиатура активна
/* USER CODE END PV */
...
// Используем прямые RGB565 значения — гарантируем совместимость
const uint16_t COLORS[7] = {
    0x07FF,  // CYAN  — I
    0xFFE0,  // YELLOW — O
    0xF81F,  // MAGENTA (фиолетовый) — T
    0x07E0,  // GREEN — S
    0xF800,  // RED — Z
    0x001F,  // BLUE — J
    0xFEA0   // ORANGE (31,28,0) — L
};
...
void draw() {
    ...
    if (gameover) {
        ...
        // Отображаем текущий режим управления
        if (joystick_mode) {
...
// ... (все остальные функции — SystemClock_Config, MX_*_Init, Error_Handler — ОСТАЮТСЯ БЕЗ ИЗМЕНЕНИЙ)
